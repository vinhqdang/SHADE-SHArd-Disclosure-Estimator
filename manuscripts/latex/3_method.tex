...
\end{equation}
While SHADE acts as the differentiable heuristic used during algorithmic optimization, ELR serves as the independent entity-level proxy for reconstruction exposure. If the ELR for a highly sensitive medical record reaches 1.0, it indicates that a single shard contains the complete context required to reconstruct that record, maximizing the probability of successful entity extraction.

\subsection{The CoShard Objective Function}
While minimizing SHADE strictly reduces disclosure risk, it inherently destroys the semantic contiguity required by RAG retrievers, severely degrading retrieval utility (demonstrating an empirical exposure-utility trade-off). Therefore, CoShard explicitly formulates semantic sharding as a constrained multi-objective optimization problem.
...
\begin{algorithmic}[1]
\REQUIRE Similarity Graph $G=(V, E, W)$, Exposure Penalty $\lambda$, Target Shards $m$
...
\WHILE{not converged and $|\mathcal{P}| > m$}
...
\ENDWHILE
\WHILE{$|\mathcal{P}| > m$}
    \STATE \textit{// Phase 3: Forced Merging to target constraint}
    \STATE Merge the two smallest adjacent shards $S_a, S_b$ that minimize $\Delta \text{SHADE}$
\ENDWHILE
...
As detailed in Algorithm \ref{alg:coshard}, Phase 1 evaluates the local move of every node $v_i$ to a neighboring shard. The move is only accepted if the increase in semantic cohesion ($\Delta Q$) outweighs the resulting topological penalty ($\lambda \cdot \Delta \text{SHADE}$). Phase 2 contracts the graph by collapsing identical shard assignments into super-nodes, reducing the search space. To enforce hardware or database topology constraints, the algorithm accepts a target shard count $m$. If the standard modularity maximization converges with $|\mathcal{P}| > m$, an explicit forced-merging phase iteratively combines the smallest adjacent shards that induce the minimal SHADE penalty until exactly $m$ shards remain, ensuring balanced enterprise deployments.
