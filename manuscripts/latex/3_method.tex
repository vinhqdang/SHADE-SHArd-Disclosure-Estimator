...
\begin{algorithmic}[1]
\REQUIRE Similarity Graph $G=(V, E, W)$, Privacy Penalty $\lambda$, Target Shards $m$
...
As detailed in Algorithm \ref{alg:coshard}, Phase 1 evaluates the local move of every node $v_i$ to a neighboring shard. The move is only accepted if the increase in semantic cohesion ($\Delta Q$) outweighs the resulting privacy penalty ($\lambda \cdot \Delta \text{SHADE}$). Phase 2 contracts the graph by collapsing identical shard assignments into super-nodes, reducing the search space. To enforce hardware or database topology constraints, the algorithm accepts a target shard count $m$. If the standard modularity maximization converges with $|\mathcal{P}| > m$, an explicit forced-merging phase iteratively combines the smallest adjacent shards that induce the minimal SHADE penalty until exactly $m$ shards remain, ensuring balanced enterprise deployments.
